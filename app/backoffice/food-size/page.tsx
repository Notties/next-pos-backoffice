...
export default function Page() {
    ...
    const fetchData = async () => {
        try {
            const res = await api.get(config.apiServer + '/api/foodSize/list');
    ...
    const remove = async (item: any) => {
        try {
            ...
            if (button.isConfirmed) {
                await api.delete(config.apiServer + '/api/foodSize/remove/' + item.id);
                fetchData();
            }
        } catch (e: any) {
            Swal.fire({
    ...
    const save = async () => {
        try {
            ...
            if (id == 0) {
                await api.post(config.apiServer + '/api/foodSize/create', payload);
            } else {
                await api.put(config.apiServer + '/api/foodSize/update', payload);
                setId(0);
            }
