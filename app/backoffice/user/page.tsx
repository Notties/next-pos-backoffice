...
export default function UserPage() {
    ...
    const fetchData = async () => {
        try {
            const response = await api.get(config.apiServer + '/api/user/list');
    ...
    const handleSave = async () => {
        try {
            ...
            if (id == 0) {
                const response = await api.post(config.apiServer + '/api/user/create', payload);
            } else {
                const response = await api.put(config.apiServer + '/api/user/update', payload);
                setId(0);
            }
    ...
    const handleDelete = async (id: number) => {
        try {
            ...
            await api.delete(config.apiServer + '/api/user/remove/' + id);
            fetchData();
        } catch (e: any) {
            Swal.fire({
                title: 'error',
