...
export default function Page() {
    ...
    const fetchDataFoodTypes = async () => {
        try {
            const res = await api.get(config.apiServer + '/api/foodType/list');
    ...
    const handleSave = async () => {
        try {
            ...
            if (id == 0) {
                await api.post(config.apiServer + '/api/food/create', payload);
            } else {
                await api.put(config.apiServer + '/api/food/update', payload);
                setId(0);
            }
    ...
    const handleUpload = async () => {
        try {
            ...
            const res = await api.post(config.apiServer + '/api/food/upload', formData);
    ...
    const handleRemove = async (id: number) => {
        try {
            ...
            if (button.isConfirmed) {
                await api.delete(config.apiServer + '/api/food/remove/' + id);
                fetchData();
            }
        } catch (e: any) {
            Swal.fire({
