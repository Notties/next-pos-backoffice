...
export default function Page() {
    ...
    const handleSave = async () => {
        try {
            ...
            if (id == 0) {
                await api.post(config.apiServer + '/api/foodType/create', payload);
            } else {
                await api.put(config.apiServer + '/api/foodType/update', payload);
                setId(0);
            }
    ...
    const handleRemove = async (item: any) => {
        try {
            ...
            if (button.isConfirmed) {
                await api.delete(config.apiServer + '/api/foodType/remove/' + item.id);
                fetchData();
            }
        } catch (e: any) {
            Swal.fire({
